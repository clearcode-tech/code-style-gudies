...
- [3] При слитии ветки задачи в ветку dev будет конфликт версии. При слитии нужно сформировать версию следующим образом:
    ...
    - По сути это всё равно что сначала слить актуальное состояние ветки dev в ветку задачи, после чего слить ветку задачи в dev (это является первым вариантом слития, описанным выше).
- [4] При фиксации версии в ветке dev после слития очередной задачи инкрементальная часть версии увеличивается на 1, суффикс задачи отбрасывается.
...
- Если условия выше не выполняются, значит кто-то другой тоже выполняет слитие веток в ветки dev. В таком случае нужно дождаться, когда этот человек закончит своё слитие в ветку dev сервиса, после чего условия по сравнению версий зависимости от jar-библиотеки должны начать выполняться.
...
1. Открыть канал для уведомлений о слитии веток задач в ветки dev и убедиться, что никто сейчас не выполняет слитий.
    ...
    - Если кто-то сейчас выполняет слития, то обозначить, что после завершения слитий этим человеком вы хотите выполнять слития. Тем самым вы как бы занимаете очередь для слития веток задач в ветки dev.

2. Если никто слитий не выполняет, то написать в канал, что вы приступаете к слитию веток задач в ветки dev.
...
5. Подтянуть последние изменения для ветки задачи и ветки dev:
...
6. Выполнить слитие ветки задачи в ветку dev согласно правилам, описанным в разделе [3]:
...
7. Выполнить фиксацию версии в dev ветке:
...
9. Если ещё есть репозитории, в которых необходимо выполнить слитие, то перейти снова к пункту 4. Если слития выполнены во всех репозиториях, то перейти дальше.

10. Написать в канал для уведомлений о слитии веток задач в ветки dev о том, что вы закончили слитие.
